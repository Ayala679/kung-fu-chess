...
public class GameLogic {
    ...
    public boolean isValid() {
        ...
        for (int i = 0; i < board.getHeight(); i++) {
            for (int j = 0; j < board.getWidth(); j++) {
                ...
                // if token != null, we assume Piece.fromToken validated it earlier
            }
        }
        return true;
    }
    ...
    private void attemptMove(Piece piece, Position from, Position to) {
        ...
        if (destination == null || destination.getColor() != piece.getColor()) {
            ...
            long duration = (pieceType == Piece.Type.N) ? GameConfig.KNIGHT_MOVE_DURATION : GameConfig.NORMAL_MOVE_DURATION;
    ...
    private boolean isMovingAt(int row, int col) {
        for (MovingPiece mp : activeMoves) {
            ...
            if (f.getRow() == row && f.getCol() == col && gameState.getCurrentTime() <= mp.getArrivalTime()) {
                return true;
            }
        }
        return false;
